...
filter = filterobject('B',[lower upper],3);
...
for i = length(list):-1:1
    ...
    for j = 1:length(list(i).channel_list);
        ...
        list(i).channels(j).template =filter_waveform_BP(list(i).channels(j).template,lower,upper);
        ...
        for time = 1:intervals;
            ...
            if isempty(w) == 1;
                ...
                w = fillgaps(w,0);
                w = filter_waveform_BP(w,upper,lower);
